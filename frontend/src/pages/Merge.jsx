...
const MergePage = () => {
  ...
  const mergePDFs = async () => {
    ...
    try {
      ...
      else alert('Error al combinar los PDF: ' + error.message)
      ...
      <section style={{ padding: '4rem 0 2rem', textAlign: 'center' }}>
        <div className="container">
          <div style={{ display: 'inline-flex', alignItems: 'center', gap: '0.5rem', padding: '0.5rem 1rem', background: 'rgba(240,147,251,0.1)', border: '1px solid rgba(240,147,251,0.3)', borderRadius: '50px', marginBottom: '1.5rem', color: '#f093fb', fontSize: '0.9rem', fontWeight: '500' }}>
            <Sparkles size={16} />Combinar varios PDF
          </div>
          <h1 style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '1rem', marginBottom: '1rem', fontSize: 'clamp(2rem,5vw,3rem)', fontWeight: '800', color: 'hsl(var(--foreground))' }}>
            ...
            Herramienta para combinar PDF
          </h1>
          <p style={{ color: 'hsl(var(--muted-foreground))', fontSize: '1.1rem', maxWidth: '600px', margin: '0 auto' }}>Combina varios archivos PDF y cambia el orden como quieras</p>
        </div>
      </section>
        ...
        <div className="container">
          <div className="grid grid-2" style={{ gap: '2rem' }}>
            <div className="glass-card" style={{ padding: '2rem' }}>
              <h3 style={{ color: 'hsl(var(--foreground))', marginBottom: '1.5rem', display: 'flex', alignItems: 'center', gap: '0.75rem', fontSize: '1.2rem', fontWeight: '700' }}>
                <div className="feature-icon-box blue" style={{ width: '40px', height: '40px', marginBottom: 0 }}><Upload size={18} /></div>Subir archivos PDF
              </h3>
              ...
              <div onClick={() => fileInputRef.current?.click()} style={{ border: '2px dashed rgba(255,255,255,0.15)', borderRadius: '8px', padding: '3rem 2rem', textAlign: 'center', cursor: 'pointer', transition: 'all 0.3s ease', marginBottom: '2rem', background: 'rgba(255,255,255,0.02)' }}
                ...
                <p style={{ color: 'hsl(var(--muted-foreground))', fontSize: '0.9rem', marginBottom: 0 }}>Selecciona varios archivos PDF para combinar</p>
              </div>

              {files.length > 0 && (
                <div>
                  <h4 style={{ color: 'hsl(var(--foreground))', marginBottom: '1rem', fontSize: '0.95rem', fontWeight: '600' }}>Archivos seleccionados ({files.length})</h4>
                  <div style={{ maxHeight: '280px', overflowY: 'auto' }}>
                    {files.map((file, index) => (
                      <div key={index} style={{ display: 'flex', alignItems: 'center', gap: '1rem', padding: '0.75rem 1rem', background: 'rgba(255,73,59,0.08)', border: '1px solid rgba(255,73,59,0.2)', borderRadius: '8px', marginBottom: '0.5rem' }}>
                  ...
                  <button onClick={mergePDFs} disabled={isLoading || files.length < 2} className="btn-glow" style={{ width: '100%', marginTop: '1.5rem', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '0.5rem', padding: '1rem 2rem' }}>
                    {isLoading ? <RefreshCw size={18} className="spin" /> : <Merge size={18} />}Combinar PDF
                  </button>
                </div>
              )}
            </div>
            ...
            <div className="glass-card" style={{ padding: '2rem' }}>
              <h3 style={{ color: 'hsl(var(--foreground))', marginBottom: '1.5rem', display: 'flex', alignItems: 'center', gap: '0.75rem', fontSize: '1.2rem', fontWeight: '700' }}>
                <div className="feature-icon-box teal" style={{ width: '40px', height: '40px', marginBottom: 0 }}><FileText size={18} /></div>Vista previa del PDF combinado
              </h3>
              {mergedPdfUrl ? (
                <div>
                  <iframe src={mergedPdfUrl} style={{ width: '100%', height: '480px', border: '1px solid rgba(255,255,255,0.15)', borderRadius: '8px' }} title="PDF combinado" />
                  <button onClick={() => { const link = document.createElement('a'); link.href = mergedPdfUrl; link.download = `merged-pdf-${Date.now()}.pdf`; link.click() }} className="btn-glow" style={{ width: '100%', marginTop: '1rem', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '0.5rem', padding: '0.75rem 1.5rem' }}>
                    <Download size={16} />Descargar PDF combinado
                  </button>
                </div>
              ) : (
                <div style={{ height: '480px', display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'rgba(255,255,255,0.02)', borderRadius: '8px', border: '2px dashed rgba(255,255,255,0.1)', color: 'hsl(var(--muted-foreground))', textAlign: 'center' }}>
                  <div>
                    ...
                    <p style={{ marginBottom: '0.5rem', fontSize: '1.1rem', fontWeight: '600' }}>Aquí verás la vista previa del PDF combinado</p>
                    <p style={{ fontSize: '0.9rem', opacity: 0.7, marginBottom: 0 }}>Sube al menos 2 archivos PDF para empezar</p>
                  </div>
                </div>
              )}
            </div>
          ...
          <div className="glass-card" style={{ marginTop: '2rem', padding: '2rem' }}>
            ...
            <div className="grid grid-3" style={{ gap: '1.5rem' }}>
              {[{ num: '1️⃣', title: 'Sube los PDF', desc: 'Haz clic o arrastra varios archivos PDF' },
              { num: '2️⃣', title: 'Ordena', desc: 'Usa los botones ↑↓ para cambiar el orden' },
              { num: '3️⃣', title: 'Combina', desc: 'Haz clic en "Combinar PDF" para unirlos y descargar' }].map((step, i) => (
